...
class LookFilePublishWidgetItem(QtGui.QWidget):
    ...
    def _populate(self, combo, data):
        items = data.get("work_paths", [])
...
class KatanaLookFilePublishPlugin(HookBaseClass):
    ...
    def __init__(self, *args, **kwargs):
        super(KatanaLookFilePublishPlugin, self).__init__(*args, **kwargs)
    ...
    def validate(self, settings, item):
        ...
        if not work_template:
            raise Exception("'{}': Work template '{}' doesn't exist".format(item.name, work_template.name))
        publish_template = item.properties["publish_template"]
        if not publish_template:
            raise Exception("'{}': Publish template '{}' doesn't exist".format(item.name, publish_template.name))
        ...
        if not os.path.exists(path):
            raise Exception("The file '{}' no longer exists on disk. Has it been deleted/published?".format(path))
        # Check the filepath is valid
        if not work_template.validate(path):
            raise Exception("The filepath '{}' does not match the template '{}'".format(path, work_template.name))
        ...
        if os.path.exists(publish_path):
            raise Exception("The file '{}' has already been copied to the publish location.".format(path))
